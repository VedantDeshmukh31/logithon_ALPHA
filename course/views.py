...
def course_registration(request):
    if request.method == "POST":
        ...
        for key in data.keys():
            course = get_object_or_404(Course, pk=key)  # Assign value to 'course'
...
def course_drop(request):
    if request.method == "POST":
        ...
        for s in range(0, len(ids)):
            ...
            # Retrieve the taken course and its credits
            taken_courses = TakenCourse.objects.filter(student=student, course=course)
